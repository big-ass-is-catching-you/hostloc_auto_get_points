...
# 登录请求
request = requests.post(login_url, data={"username": username, 'password': password}, impersonate="chrome")
...
# 获取用户积分中心
user_info = requests.get('https://hostloc.com/home.php?mod=spacecp&ac=credit', impersonate="chrome", cookies=request.cookies).text
...
# 正题，刷积分
for i in range(50780, 50800):
    request1 = requests.get(f'https://hostloc.com/space-uid-{i}.html', impersonate="chrome", cookies=request.cookies)
    ...
    time.sleep(6)  # 可选的延时
    new_money_info = requests.get('https://hostloc.com/home.php?mod=spacecp&ac=credit', impersonate="chrome", cookies=request.cookies).text
